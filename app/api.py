...
from .security import create_otp, verify_otp, create_session, current_user
...
def request_otp(body:Identifier, db:Session=Depends(get_db)):
    ident=body.identifier.strip().lower(); challenge, code=create_otp(db,ident)
    delivery=__import__("os").getenv("OTP_DELIVERY","disabled")
    if delivery=="log": return {"status":"issued","challenge_id":challenge.id,"development_code":code}
    if delivery!="disabled": raise HTTPException(503,"Configured OTP delivery provider is not supported by this build.")
    return {"status":"issued","challenge_id":challenge.id,"delivery":"not_configured","message":"Set a real OTP delivery provider before production use."}
...
def me(authorization:str|None=Header(default=None),db:Session=Depends(get_db)):
    u=auth_user(authorization,db); return {"id":u.id,"identifier":u.identifier}
...
def add_medicine(body:MedicineIn,authorization:str|None=Header(default=None),db:Session=Depends(get_db)):
    ...
    row=Medicine(user_id=u.id,name=body.name,rxcui=rxcui,ingredient=body.ingredient,strength=body.strength,notes=body.notes)
    db.add(row); db.add(AuditEvent(user_id=u.id,event="cabinet.add")); db.commit(); db.refresh(row)
...
def add_reminder(body:ReminderIn,authorization:str|None=Header(default=None),db:Session=Depends(get_db)):
    ...
    r=Reminder(user_id=u.id,medicine_id=body.medicine_id,title=body.title,schedule=body.schedule,timezone_name=body.timezone_name)
    db.add(r); db.add(AuditEvent(user_id=u.id,event="reminder.create")); db.commit(); db.refresh(r)
    return {"id":r.id,"title":r.title,"schedule":r.schedule,"enabled":r.enabled,"delivery":"worker_ready_provider_required"}
...
async def identify_photo(file:UploadFile=File(...)):
    ...
    candidates=[]
    for med in parse_prescription(ocr.get("text","")).get("medications",[]):
        candidates.append({**med,"rxnorm_candidates":rxnorm_search(med["candidate_name"])[:5],"verified":False})
...
def india_medicine(name:str):
    return {"query":name,"global_concepts":rxnorm_search(name)[:10],"india_resources":india_links(name),"notice":"India availability, brand identity, prescription status and pricing require verification against the local product/package and authorized pharmacy/regulatory sources."}
