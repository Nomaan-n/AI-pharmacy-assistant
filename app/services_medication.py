...
def rxnorm_search(term):
    data=get_json(f"{RXNORM}/drugs.json",{"name":term})
    out=[]; seen=set()
    for group in (data or {}).get("drugGroup",{}).get("conceptGroup",[]):
        for p in group.get("conceptProperties",[]) or []:
            n=p.get("name")
            if n and n.lower() not in seen:
                seen.add(n.lower()); out.append({"name":n,"rxcui":p.get("rxcui"),"tty":p.get("tty"),"source":"NIH RxNorm"})
    return out[:20]

def rxnorm_exact(term):
    data=get_json(f"{RXNORM}/rxcui.json",{"name":term,"search":0})
    ids=(data or {}).get("idGroup",{}).get("rxnormId",[]) or []
    return ids[0] if ids else None

def rxnorm_properties(rxcui):
    data=get_json(f"{RXNORM}/rxcui/{rxcui}/properties.json")
    return (data or {}).get("properties",{})
...
def india_links(term):
    q=quote_plus(term)
    return [
      {"name":"CDSCO","region":"India","url":"https://cdsco.gov.in/opencms/opencms/en/"},
      {"name":"Tata 1mg","region":"India","url":f"https://www.1mg.com/search/all?name={q}"},
      {"name":"PharmEasy","region":"India","url":f"https://pharmeasy.in/search/all?name={q}"},
      {"name":"Apollo Pharmacy","region":"India","url":f"https://www.apollopharmacy.in/search-medicines?query={q}"},
      {"name":"Netmeds","region":"India","url":f"https://www.netmeds.com/products?search={q}&verticalspecification=Medicine"}]
